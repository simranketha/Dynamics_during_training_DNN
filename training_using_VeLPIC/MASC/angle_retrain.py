...
def peak_info_cnnnetwork(peak_epoch):
...
def results_folder(type_network,ds,run,corrupt,data_out,load_epoch=10,n=1,once_relabel=False,
                   model=False,twice_relabel=False,model_masc=False,old=False):
    if once_relabel:
        folder_relabel='once_relabel'
    elif twice_relabel:
        folder_relabel='twice_relabel'
    else:
    ...
    if model:
        ...
        pred_model='model_masc'
    else:
        pred_model='masc'
    
    
    if old:
        data='MASC_retraining'
    else:
        data='MASC_retraining_again'
        
    b_path=f'{data}/{angle}/{folder_relabel}'
    results=f'{b_path}/{load_epoch}/{type_network}_{ds}/{corrupt}/Run_{run}/{pred_model}/{data_out}'
    os.makedirs(f'{results}',exist_ok=True)

    b_path=f'/mnt/SSD2TB/simran/PCA/{data}/{angle}/{folder_relabel}'
...
def results_model(type_network,ds,run,corrupt,load_epoch=10,n=1,once_relabel=False,
                  model=False,twice_relabel=False,model_masc=False,old=False):
    if once_relabel:
        folder_relabel='once_relabel'
    elif twice_relabel:
