...
# # WE WILL CHANGE THE WEIGHT OF THE LAST LAYER OF THE MODEL WITH MAVC AT A SPECIFIC EPOCH AND THEN TRAIN the model
...
#     for run in range(1,runs): 
    results,run_path,temp_path=results_folder_MAVC(type_network,ds,run,corrupt,p_layer,
                                                   peak_epoch,once_relabel=True)
    print(results,run_path,temp_path)

    if type_network=='CNN' and ds=='MNIST' and peak_epoch==40 and run==3 and corrupt==0.0:
        epoch,interval=peak_info_cnnnetwork(peak_epoch)
    else:
        epoch,interval=peak_info(peak_epoch)
    print(epoch)
    ...
    loss_func,optimizer,batch_size=cnn_create.model_params(type_network,ds,
                                                           dummy_model,dev=dev)
    ...
    for epoch_number in list(range(peak_epoch,epochs,interval)):

        dummy_model=training_process(corrupted_train,train_loader,test_loader,
                                     dummy_model,loss_func,optimizer,run_path,
                                     results,dev,epoch_number,interval)
