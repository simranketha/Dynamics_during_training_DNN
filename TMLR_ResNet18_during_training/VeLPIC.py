...
def temp_store(ds,type_network,corrupt,run,n_value,seed_value,epoch,layer_number):
    ...
    temp_path = f'Network_data/velpic_{ds}_{type_network}_{corrupt}_{run}_{seed_value}_{epoch}_{layer_number}'
    os.makedirs(temp_path,exist_ok=True)
      
    results_folder=f'results_velpic/{seed_value}/angle_results_{n_value}/{ds}_{type_network}'
...
#         model_numbers=sorted_model_numbers(network_path,corrupt,run)
    ...
    for model_epoch in model_numbers:
        ...
        del dummy_model
        mavc.MAVC_fn_neg_pca_ResNet18(temp_path,run,results_corr,epoch,subspace_type,num_class,layer_number)
