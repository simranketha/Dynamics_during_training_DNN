...
from MASC import MAVC_pytorch as mavc
import os
import warnings
...
def weight_change(dummy_model,final_tensor,withbias=False,type_network='MLP'):
    networks_bias=['MLP' ,'AlexNet','ResNet18']
    if type_network in networks_bias:
        if withbias:
            if type_network=='MLP':
                ...
                dict_withbias=copy.deepcopy(dummy_model.fc.state_dict())
                dict_withbias['weight']=final_tensor
                dummy_model.fc.load_state_dict(dict_withbias)
            ...
            if type_network=='MLP':
                ...
                dict_withoutbias=copy.deepcopy(dummy_model.fc.state_dict())
                dict_withoutbias['weight']=final_tensor
                dict_withoutbias['bias']=torch.zeros(dict_withoutbias['bias'].shape[0], 
                                                     dtype=torch.float32,
                                                     device='cuda')
                dummy_model.fc.load_state_dict(dict_withoutbias)
...
def results_store(type_network):
    if type_network=='MLP' or type_network=='AlexNet' or type_network=='ResNet18':
            results = {
              'epoch': [],
              'Train_acc_withbias': [],
...
#         args.print_help()
    ...
    if type_network == 'ResNet18':
        layer_number=8
        num_class=10
        d_layer,p_layer=mavc.layer_name_details(layer_number)
        general_pca='/mnt/8TB/simran/MASC/pca_saved/TMLR_compare/40/angle_results_1'
        path_pca=f'{general_pca}/{ds}_{type_network}/results_{corrupt}/pca_corrupted'
    else:
...
#     for run in range(1,runs): 
    temp_path = f'VeLPIC_WEIGHTCHANGE_Network_data_{corrupt}_{ds}_{type_network}_{run}'
    ...
    for epoch in cnn_create.selected_model_list(network_path,corrupt,run):
        ...
        if type_network=='ResNet18':
            cnn_create.ResNet18_loading_saving_activations_layerwise(temp_path,dummy_model,corrupted_train,test_loader,
                                       og_targets,dev,type_network,layer_number)
        else:
            cnn_create.saving_activations_lastlayer(temp_path,dummy_model,corrupted_train,test_loader,
        ...
        if type_network=='ResNet18':
            layer_output=mavc.data_only_train(temp_path, d_layer)
        else:
