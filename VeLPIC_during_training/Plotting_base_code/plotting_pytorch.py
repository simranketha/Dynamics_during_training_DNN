...
def path(network,data_type,corrupt,n,mavc=False,train=False):
    if mavc:
        if train:
            typex=f'results/MAVC_angle_results_1pc_train/{data_type}_{network}/results_{corrupt}'
            type_acc=f'../pca_during_training_angle/angle_results/{data_type}_{network}/Accuracy_results/{corrupt}'
        else:
            typex=f'results/MAVC_angle_results_1pc/{data_type}_{network}/results_{corrupt}'
            type_acc=f'../pca_during_training_angle/angle_results/{data_type}_{network}/Accuracy_results/{corrupt}'
...
def path_weight(network,data_type,corrupt):
    ...
    typex=f'results/MAVC_weight_change/{data_type}_{network}/results_{corrupt}'
    type_acc=f'../pca_during_training_angle/angle_results/{data_type}_{network}/Accuracy_results/{corrupt}'
       
    return typex,type_acc

def path_angle(network,data_type,corrupt):
    typex=f'results/MAVC_angle_weight/{data_type}_{network}/results_{corrupt}'
