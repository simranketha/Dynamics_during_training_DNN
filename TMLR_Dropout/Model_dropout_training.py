...
#         for run in range(2,runs):
    ...
    print(path,network_path,res_path)
    results = {
                'epoch': [],
                'Train_accuracy': [],
                'Test_accuracy': [],
    ...
    if type_network=='ResNet18':
        ...
        model = cnn_create.ResNet18_drop()
        loss_func = nn.CrossEntropyLoss()
        optimizer = optim.SGD(model.parameters(), lr=0.001, momentum=0.9)
    else:
        model,loss_func,optimizer,batch_size=cnn_create.model_build_drop(type_network,ds,dropout=dropout)
    model.to(device)
