...
#       ----- flops calculation
    ...
    if n>=1:
        ...
        df1.to_csv(f"{path2_pca}/pca_train_{subspace_type}_{data_out}_var.csv")
    ...
    df_flops.to_csv(f"{path2_pca}/pca_train_{subspace_type}_{data_out}_flops.csv", index=False)
...
def masc_project_test(temp_path,path2_pca,data,data_out,num_class,dev,subspace_type):
        #testing images
    ...
    for class_projection in range(0,num_class,1):
        X_ppca=torch.load(f"{path2_pca}/pca_train_{subspace_type}_{data_out}_{class_projection}.pt")       
...
def masc_probe(type_network,ds,temp_path,run,results,n,epoch_present,dev,subspace_type,layer_name=None):
    ...
    with torch.no_grad(): 
        for data,data_out in zip(data_layer_name,data_output_name):
            ...
            acc_overall,flops_infer=masc_project_test(temp_path,path2_pca,data,data_out,
                                                      num_class,dev,subspace_type)
