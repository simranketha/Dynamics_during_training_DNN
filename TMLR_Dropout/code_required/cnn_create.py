...
class NgnCnn(nn.Module):
    ...
    def get_intermediate_states(self):
        return [self.input,self.out_conv1 , 
              self.out_conv2 ,
              self.out_conv3,
              self.out_conv4 ,
              self.out_conv5,
...
class Bottleneck(nn.Module):
    expansion = 4  # Output channels are 4 times base channels
...
def get_cifar_dataloaders_corrupted(corrupt_prob=0, batch_size=50,fashion=False,cifar10=False,
    ...
    data_foldername='~/work/data'
    data_foldername_tiny='~/work/data/tiny-imagenet-200'
    data_foldername_imagenet='/mnt/2TB/DeepLearningExamples/PyTorch/Classification'
    if fashion:
        transform = transforms.Compose(
        [transforms.ToTensor(),
         transforms.Normalize((0.5,), (0.5,))])
...
def data_saving_lastlayer(path,loader,dummy_model,name,dev,type_network):
    ...
    if type_network=='ResNet18':
        ...
        extractor = create_feature_extractor(dummy_model,
        ...
        data_layer_name=['before_fc',
                         'y_value_corrupted']
        outputs = [output8, y_value] 
    if type_network=='AlexNet':
        dummy_output =None
        output3=[]
        y_value=[]
...
def loading_saving_activations(result_path,dummy_model,corrupted_train,test_loader,og_targets,dev,type_network):
    #training data
...
def loading_saving_activations_test(result_path,dummy_model,test_loader,og_targets,dev,type_network):
    #testing data
    data_saving(result_path,test_loader,dummy_model,'test',dev,type_network)   
    
def original_dataset(type_network,ds,corrupt,batch_size=1):
    if type_network=='MLP':
        if ds=='MNIST':
            network_path = '/mnt/NVME2TB/simran/PCA/MLP_MNIST/Network'
            _, test_loader , _,_,_=get_cifar_dataloaders_corrupted(batch_size=batch_size,mnist=True)
            corrupted_train, _ , _, og_targets, _ = get_cifar_dataloaders_corrupted(corrupt_prob=corrupt, batch_size=batch_size,mnist=True)

        if ds=='CIFAR10':
            network_path = '/mnt/NVME2TB/simran/PCA/MLP_CIFAR10/Network'
            _, test_loader , _,_,_= get_cifar_dataloaders_corrupted(batch_size=batch_size,cifar10=True)
            corrupted_train, _ , _, og_targets, _ = get_cifar_dataloaders_corrupted(corrupt_prob=corrupt, batch_size=batch_size,cifar10=True)
    elif type_network =='CNN':
        if ds=='FashionMNIST':
            network_path = '/mnt/NVME2TB/simran/PCA/FashionMNIST/Network'
            _, test_loader , _,_,_= get_cifar_dataloaders_corrupted(batch_size=batch_size,fashion=True)
            corrupted_train, _ , _, og_targets, _ = get_cifar_dataloaders_corrupted(corrupt_prob=corrupt, batch_size=batch_size,fashion=True)
        if ds=='MNIST':
        ...
        if ds=='TinyImageNet':
            ...
            corrupted_train, _ , _, og_targets, _ = get_cifar_dataloaders_corrupted(corrupt_prob=corrupt, batch_size=batch_size,tiny_imagenet=True)
    elif type_network=='ResNet18':
        if ds=='CIFAR10':
            network_path = '/home/simrank/work/TMLR_compare/Modern_backbones/models/CIFAR10_ResNet18/Network'
            _, test_loader , _,_,_= get_cifar_dataloaders_corrupted(batch_size=batch_size,cifar10=True)
            corrupted_train, _ , _, og_targets, _ = get_cifar_dataloaders_corrupted(corrupt_prob=corrupt, batch_size=batch_size,cifar10=True)
    return network_path,test_loader,corrupted_train,og_targets
...
def model_create(type_network,ds):    
    if type_network=='CNN':
        if ds=='CIFAR10':
            dummy_model = NgnCnn()
        else:
            dummy_model = NgnCnn(channels=1)
        ...
        dummy_model = AlexNet(
            num_classes=200, tiny_imagenet=True)
    elif type_network=='ResNet18': 
        dummy_model = ResNet18()
    return dummy_model

def model_params(type_network,ds,model,dev="cuda",l_r=False):
    if type_network=='CNN':
        ...
        batch_size=32
    return loss_func,optimizer,batch_size




def select_model(values,tiny=False):
    selected_values = []
...
#     def __init__(self, classes=10, mnist=True, dropout=0.0):
#         super().__init__()

#         in_dim = 28 * 28 if mnist else 32 * 32 * 3
...
#         self.after_drop_fc1 = None
#         self.after_drop_fc2 = None
#         self.after_drop_fc3 = None
#         self.after_drop_fc4 = None

#     def forward(self, x):
#         # Input
#         self.input_img = x.detach().clone()

#         x = torch.flatten(x, 1)
#         self.after_flatten = x.detach().clone()

#         # FC1
#         x = F.relu(self.fc1(x))
#         self.after_relu_fc1 = x.detach().clone()
#         x = self.drop1(x)
#         self.after_drop_fc1 = x.detach().clone()

#         # FC2
#         x = F.relu(self.fc2(x))
#         self.after_relu_fc2 = x.detach().clone()
#         x = self.drop2(x)
#         self.after_drop_fc2 = x.detach().clone()

#         # FC3
#         x = F.relu(self.fc3(x))
#         self.after_relu_fc3 = x.detach().clone()
#         x = self.drop3(x)
#         self.after_drop_fc3 = x.detach().clone()

#         # FC4
#         x = F.relu(self.fc4(x))
#         self.after_relu_fc4 = x.detach().clone()
#         x = self.drop4(x)
#         self.after_drop_fc4 = x.detach().clone()
...
def network_path_dropout(type_network,ds):
    if type_network=='MLP' or type_network=='CNN':
        network_path=f'models/{type_network}_{ds}_dropout/Network'

    elif type_network=='AlexNet' and ds=='CIFAR100':
        network_path='models/CIFAR_100_dropout/Network'
    return network_path


def model_build_drop(type_network,ds,dropout=False):
    if dropout:
        dropvalue=0.2
    else:
        dropvalue=0
    if type_network=='CNN':
        if ds=='CIFAR10':
            ...
            batch_size = 128
        optimizer = torch.optim.Adam(dummy_model.parameters(),lr=0.0002)
        loss_func = nn.CrossEntropyLoss()

#     elif type_network=='MLP':
#         if ds=='CIFAR10':
#             dummy_model = mlp_drop(mnist=False,dropout=dropvalue)
#             optimizer = torch.optim.Adam(dummy_model.parameters(), lr=1e-3)
# #             optimizer = torch.optim.Adam(dummy_model.parameters(), lr = 1.5e-3)


#         else:
#             dummy_model = mlp_drop(mnist=True,dropout=dropvalue) 
# #             optimizer = torch.optim.Adam(dummy_model.parameters(), lr = 1.5e-3)
#             optimizer = torch.optim.Adam(dummy_model.parameters(), lr=1e-3)
...
#             optimizer=optim.Adam(dummy_model.parameters(),betas=(0.9, 0.999),lr = 0.0001)
#             loss_func=nn.CrossEntropyLoss().to(device)

#         if ds=='CIFAR100':
#             dummy_model = AlexNet(num_classes=100,dropout=dropout)
#             optimizer = torch.optim.Adam(dummy_model.parameters(),lr=0.0001)
#             loss_func = nn.CrossEntropyLoss()
    return dummy_model,loss_func,optimizer,batch_size
def data_saving_lastlayer_dropout(path,loader,dummy_model,name,dev,type_network):
    if type_network=='CNN':
        dummy_output =None
        output4=[]
        y_value=[]
        ...
        for x,y in loader:
            ...
            res = dummy_model.get_intermediate_states()
            out_fcs=res[-1]
            output4.append(out_fcs[0].cpu())
            y_value.append(y[0])  
        data_layer_name=[ 'before_output','y_value_corrupted']
        outputs = [output4, y_value]
...
def saving_activations_lastlayer_dropout(result_path,dummy_model,corrupted_train,test_loader,
                                         og_targets,dev,type_network):
    #training data
    data_saving_lastlayer_dropout(result_path,corrupted_train,dummy_model,'train',dev,type_network)
    ...
    #testing data
    data_saving_lastlayer_dropout(result_path,test_loader,dummy_model,'test',dev,type_network)   
...
def testing_model(model,loss_func,loader,dev):
    ...
    return test_acc   
def epoch_inference(corrupted_train,test_loader,
                    model,loss_func,epoch_present,results_path,run,dev):
    results_dic = {
      'epoch': [],
      'Train_accuracy': [],
       'Test_accuracy': [],
    ...
    file_name=f"{results_path}/Accuracy_Run_{run}.csv"
...
class ResNet18_drop(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.relu(out)
        out = self.maxpool(out)
        self.after_layer_0 = out.detach().clone()

        out = self.layer1(out)
        self.after_layer_1 = out.detach().clone()

        out = self.layer2(out)
        self.after_layer_2 = out.detach().clone()

        out = self.layer3(out)
        self.after_layer_3 = out.detach().clone()

        out = self.layer4(out)
        self.after_layer_4 = out.detach().clone()

        out = self.avgpool(out)
        out = out.view(out.size(0), -1)

        self.before_fc = out.detach().clone()

        out = self.dropout(out)
        self.after_dropout = out.detach().clone()
...
def path_model(ds,type_network,dropout):
    ...
    elif type_network=='CNN' and ds=='FashionMNIST':
        if dropout:
            path=f'models/FashionMNIST_d_0_2'
    
    elif type_network=='CNN' and ds=='CIFAR10':
        if dropout:
            path=f'models/CIFAR_10_CNN'
    
    elif type_network=='CNN' and ds=='MNIST':
        if dropout:
            path=f'models/{ds}_{type_network}_dropout'  
...
def  ResNet18_layer_details_dropout(dummy_model,layer_number):
        
    if layer_number==0:
        return_nodes = {'maxpool': 'after_layer_0'}
        extractor = create_feature_extractor(dummy_model, return_nodes=return_nodes)
        layer_name='after_layer_0'
    elif layer_number==1:
        return_nodes = {'layer1.0.relu':'after_layer_0_1'}
        extractor = create_feature_extractor(dummy_model, return_nodes=return_nodes)
        layer_name='after_layer_0_1'    
    elif layer_number==2:
        return_nodes = {'layer1.0.relu_1':'after_layer_0_2'}
        extractor = create_feature_extractor(dummy_model, return_nodes=return_nodes)
        layer_name='after_layer_0_2'    
    elif layer_number==3:
        return_nodes = {'layer1.1.relu':'after_layer_0_3'}
        extractor = create_feature_extractor(dummy_model, return_nodes=return_nodes)
        layer_name='after_layer_0_3'    
    elif layer_number==4:
        return_nodes = {'layer1.1.relu_1':'after_layer_1'}
        extractor = create_feature_extractor(dummy_model, return_nodes=return_nodes)
        layer_name='after_layer_1' 
    elif layer_number==5:
        return_nodes = {'layer2.1.relu_1':'after_layer_2'}
        extractor = create_feature_extractor(dummy_model, return_nodes=return_nodes)
        layer_name='after_layer_2'     
    elif layer_number==6:
        return_nodes = {'layer3.1.relu_1':'after_layer_3'}
        extractor = create_feature_extractor(dummy_model, return_nodes=return_nodes)
        layer_name='after_layer_3'      
    elif layer_number==7:
        return_nodes = {'layer4.1.relu_1':'after_layer_4' }
        extractor = create_feature_extractor(dummy_model, return_nodes=return_nodes)
        layer_name='after_layer_4'   
    elif layer_number==8:
        return_nodes = {'view':'before_fc'}
        extractor = create_feature_extractor(dummy_model, return_nodes=return_nodes)
        layer_name='before_fc'
    elif layer_number==9:
        return_nodes = {'view':'after_dropout'}
        extractor = create_feature_extractor(dummy_model, return_nodes=return_nodes)
        layer_name='after_dropout'
    return extractor,layer_name
def data_saving_ResNet18_layerwise_dropout(path,loader,dummy_model,name,dev,type_network,layer_number):
    
    output0=[]
    y_value=[]
    dummy_model.to(dev)
    extractor,layer_name=ResNet18_layer_details_dropout(dummy_model,layer_number)
        
    for x,y in loader:

        dummy_out_fcs = extractor(x.to(dev))
        if layer_number==8:
            output0.append(dummy_out_fcs['before_fc'].view(-1).detach().cpu().numpy())
        else:
            output0.append(dummy_out_fcs[layer_name].view(-1).detach().cpu())
        y_value.append(y[0])

    data_layer_name=[layer_name,
                     'y_value_corrupted']

    outputs = [output0, y_value] 
    for idx,data in enumerate(data_layer_name):
        if 0 <= idx < len(outputs):
            data_value = outputs[idx]
        with open(f'{path}/{data}_{name}.pkl', 'wb') as file: 
...
def ResNet18_loading_saving_activations_layerwise_dropout(result_path,dummy_model,corrupted_train,test_loader,
                               og_targets,dev,type_network,layer_number):
    #training data
    data_saving_ResNet18_layerwise_dropout(result_path,corrupted_train,dummy_model,'train',dev,type_network,layer_number)


    #testing data
    data_saving_ResNet18_layerwise_dropout(result_path,test_loader,dummy_model,'test',dev,type_network,layer_number)

def ResNet18_loading_saving_activations_layerwise_test_dropout(result_path,dummy_model,test_loader,dev,type_network,layer_number):

    #testing data
    data_saving_ResNet18_layerwise(result_path,test_loader,dummy_model,'test',dev,type_network,layer_number)

            
def data_saving_cnn_dropout(path,loader,dummy_model,name,dev,type_network):
...
#         output0.append(x.detach().cpu().numpy().reshape(-1))
        ...
        output1.append(out_fcs['input_fc_0'][0].cpu())
        output2.append(out_fcs['output_fc_0_after_dropout'][0].cpu())
        output3.append(out_fcs['output_fc_1_after_dropout'][0].cpu())
        output4.append(out_fcs['output_fc_2_after_dropout'][0].cpu())
        output5.append(out_fcs['output_fc_0_after_noise_relu'][0].cpu())
        output6.append(out_fcs['output_fc_1_after_noise_relu'][0].cpu())
        output7.append(out_fcs['output_fc_2_after_noise_relu'][0].cpu())
        y_value.append(y[0])  
    data_layer_name=['input_fc_0','output_fc_0_after_noise_relu',
                     ...
                     'y_value_corrupted']#'input_layer',
    outputs = [ output1, output2, output3, output4, output5, output6, output7, y_value]
#     output0,
    for idx,data in enumerate(data_layer_name):
        if 0 <= idx < len(outputs):
            data_value = outputs[idx]
...
def loading_saving_activations_cnn_dropout(result_path,dummy_model,corrupted_train,test_loader,dev,type_network):
    #training data
    data_saving_cnn_dropout(result_path,corrupted_train,dummy_model,'train',dev,type_network)

    #testing data
    data_saving_cnn_dropout(result_path,test_loader,dummy_model,'test',dev,type_network)
